...
class OrderItemInline(admin.TabularInline):
    ...
    def image_preview(self, obj):
        if obj.product.main_image:
            return mark_safe(
                f'<img src="{obj.product.main_image.url}" style="max-height: 100px; '
                f'max-width: 100px; object-fit: cover;" />'
            )
...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('created_at', 'updated_at', 'total_price', 'payment_provider', 'stripe_payment_intent_id')
    inlines = [OrderItemInline]

    fieldsets = (
        ("Order Information", {
            'fields': ('user', 'first_name', 'last_name', 'email', 'company', 'address1', 'address2', 'city', 'country',
                       'province', 'postal_code', 'phone', 'special_instructions', 'total_price'),
        }),
        ('Payment and Status', {
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj:
            return self.readonly_fields + ('user', 'first_name', 'last_name', 'email', 'company', 'address1',
