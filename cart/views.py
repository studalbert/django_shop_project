...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if product_size.stock < quantity:
            return JsonResponse({'error': f'Only {product_size.stock} {product_size.size.name} left in stock'}, status=400)
        ...
        if request.headers.get('HX-Request'):
            ...
            return JsonResponse({
                ...
                'message': f'{product.name} ({product_size.size.name}) added to cart',
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({'error': f'Only {cart_item.product_size.stock} {cart_item.product_size.size.name} left in stock'}, status=400)
...
class ClearCartView(CartMixin, View):
    ...
    def post(self, request):
        ...
        request.session.modified = True
        if request.headers.get('HX-Request'):
            return TemplateResponse(request, 'cart/cart_empty.html', context={'cart': cart})
